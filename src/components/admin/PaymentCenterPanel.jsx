...
function PaymentCenterPanel({
  ...
  async function handleAddPayment(event) {
    ...
    try {
      ...
      const confirmed = window.confirm(
        ...
        }\n\nOnly continue if money was actually received.`
      );
      ...
      showMessage(
        "Payment recorded. The database has reconciled the linked invoice automatically.",
        "success"
      );
      await refreshAll();
    } catch (error) {
  ...
  async function changePaymentStatus(payment, nextStatus) {
    ...
    if (current === normalize(nextStatus)) return;
    ...
    if (!window.confirm(labels[nextStatus] || `Change payment to ${nextStatus}?`)) {
      return;
    }
    ...
    try {
      ...
      await createTimeline(
        `Payment ${nextStatus.replaceAll("_", " ")}`,
        `${formatMoney(
          payment.amount,
          payment.currency
        )} payment changed from ${payment.status || "unknown"} to ${nextStatus}.`,
        {
      ...
      showMessage(
        `Payment marked ${nextStatus.replaceAll("_", " ")}. Invoice totals were recalculated automatically.`,
        "success"
      );
      await refreshAll();
    } catch (error) {
  ...
  async function changeInvoiceStatus(invoice, nextStatus) {
    ...
    const wording =
      nextStatus === "void"
        ? "Void this invoice? Use this when the invoice should never have been collectible."
        : nextStatus === "cancelled"
          ? "Cancel this invoice? It will remain in the finance history but will no longer be collectible."
    ...
    try {
      ...
      await createTimeline(
        `Invoice ${nextStatus.replaceAll("_", " ")}`,
      ...
      showMessage(`Invoice marked ${nextStatus}.`, "success");
      await refreshAll();
    } catch (error) {
  ...
  async function updateReceiptStatus(receipt, nextStatus) {
    ...
    if (nextStatus === "approved" && Number(receipt.amount || 0) <= 0) {
    ...
    const confirmed = window.confirm(
      nextStatus === "approved"
        ? `Approve this receipt and create a confirmed payment for ${formatMoney(
            receipt.amount,
            receipt.currency
          )}?`
        : "Reject this receipt? No payment will be created."
    );
    ...
    try {
      ...
      if (invoice && !isCollectibleInvoice(invoice)) {
        throw new Error(
      ...
      showMessage(error?.message || "Receipt could not be reviewed.", "error");
    } finally {
