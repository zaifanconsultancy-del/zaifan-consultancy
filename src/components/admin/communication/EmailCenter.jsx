...
const workflows = ["Inquiry nurture sequence", "Offer received follow-up", "CAS readiness checklist", "Payment reminder"].map((name, index) => ({ id: index + 1, name, owner: ["Ayesha", "Hamza", "Muneeb", "Sara"][index % 4], sla: ["15m", "2h", "1d", "3d"][index % 4], status: ["Active", "Review", "Queued", "Needs Update"][index % 4], success: [94, 88, 81, 76][index % 4] }));
const categories = ["New leads", "Applications", "Offers", "CAS/Visa", "Finance"];
...
export default function EmailCenter({ compact = false }) {
  ...
  return <div className="space-y-6">
    ...
    <div className={`${card} p-5`}>
      <SectionHeader title="Email Center" description="Email operations, templates, inbox triage, campaign queues, and SLA monitoring." action={<button className="rounded-xl bg-slate-950 px-4 py-2 text-sm font-bold text-white">New Workflow</button>} />
