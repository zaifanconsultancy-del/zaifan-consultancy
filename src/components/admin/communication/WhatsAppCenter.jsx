...
const workflows = ["Document missing reminder", "Visa appointment prep", "University shortlist message", "Payment receipt follow-up"].map((name, index) => ({ id: index + 1, name, owner: ["Ayesha", "Hamza", "Muneeb", "Sara"][index % 4], sla: ["15m", "2h", "1d", "3d"][index % 4], status: ["Active", "Review", "Queued", "Needs Update"][index % 4], success: [94, 88, 81, 76][index % 4] }));
const categories = ["Hot leads", "Active students", "Visa stage", "Payment due", "Dormant"];
...
export default function WhatsAppCenter({ compact = false }) {
  ...
  return <div className="space-y-6">
    ...
    <div className={`${card} p-5`}>
      <SectionHeader title="WhatsApp Center" description="Fast student conversations, broadcast queues, counselor routing, and response quality." action={<button className="rounded-xl bg-slate-950 px-4 py-2 text-sm font-bold text-white">New Workflow</button>} />
