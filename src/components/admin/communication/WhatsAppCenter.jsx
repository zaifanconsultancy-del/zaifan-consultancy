// WhatsAppCenter V4 PARTNER-OS STANDARD
// Full replacement for: src/components/admin/communication/WhatsAppCenter.jsx
...
export default function WhatsAppCenter({
  ...
  const channelData = communicationData?.whatsapp || {};
  ...
  function handlePrimary(record) {
    ...
    const digits = getPhone(record).replace(/\D/g, ""); if (!digits) return; const normalizedPhone = digits.startsWith("92") ? digits : digits.startsWith("0") ? `92${digits.slice(1)}` : digits; window.open(`https://wa.me/${normalizedPhone}`, "_blank", "noopener,noreferrer");
  }

  const primaryEnabled =
    typeof onOpenWhatsApp === "function" || true;
  ...
  return (
    <Shell
      icon={MessageCircle}
      osLabel="WhatsApp OS"
      title="WhatsApp Communication Command"
      description="Open genuine student WhatsApp conversations from real CRM phone records without pretending that a full messaging inbox exists."
      workspace="WhatsApp Operations"
      adminProfile={adminProfile}
      recordsCount={stats.total}
      secondaryCount={telemetryConnected ? "Telemetry connected" : "CRM-only mode"}
    >
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="WhatsApp Ready" value={stats.total} helper="Real CRM records with a usable contact value." tone="navy" icon={MessageCircle} badge="Contactability" />
      ...
      <section className="rounded-[1.5rem] border-[3px] border-[#C9D7E6] bg-white p-4 sm:p-5">
        <div className="mb-4 flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
          <div>
            <p className="text-[9px] font-black uppercase tracking-[0.11em] text-orange-700">WhatsApp Operations</p>
        ...
        <div className="space-y-2.5">
          {visibleRecords.length ? (
              ...
              <article key={record.id || `${getName(record)}-${index}`} className="rounded-[1.3rem] border-2 border-[#C9D7E6] bg-white p-4 shadow-[0_6px_18px_rgba(15,35,63,0.04)] transition hover:border-[#F97316]">
                <div className="grid gap-4 xl:grid-cols-[minmax(18rem,1.4fr)_minmax(10rem,0.8fr)_minmax(8rem,0.6fr)_minmax(10rem,0.75fr)_minmax(10rem,0.8fr)] xl:items-center">
                  ...
                  <div className="col-span-full flex flex-wrap items-center justify-end gap-2 border-t-2 border-[#E7EDF4] pt-3">
                    ...
                    <button type="button" onClick={() => handlePrimary(record)} disabled={!primaryEnabled} className="inline-flex min-h-9 items-center gap-2 rounded-xl border-2 border-[#123865] bg-[#123865] px-3 text-[10px] font-black text-white disabled:opacity-40"><Send size={13} /> Open WhatsApp</button>
                  </div>
                </div>
              </article>
            ))
            ...
            <div className="rounded-[1.4rem] border-[3px] border-dashed border-[#C9D7E6] bg-[#FFF8EF] p-8 text-center">
              <MessageCircle size={24} className="mx-auto text-orange-700" />
