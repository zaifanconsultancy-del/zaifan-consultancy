...
const workflows = ["Initial counseling", "Application review", "Visa mock interview", "Parent finance meeting"].map((name, index) => ({ id: index + 1, name, owner: ["Ayesha", "Hamza", "Muneeb", "Sara"][index % 4], sla: ["15m", "2h", "1d", "3d"][index % 4], status: ["Active", "Review", "Queued", "Needs Update"][index % 4], success: [94, 88, 81, 76][index % 4] }));
const categories = ["Today", "This week", "Follow-up due", "No-show", "Completed"];
...
export default function MeetingCenter({ compact = false }) {
  ...
  return <div className="space-y-6">
    ...
    <div className={`${card} p-5`}>
      <SectionHeader title="Meeting Center" description="Counseling sessions, admission reviews, visa prep meetings, and follow-up ownership." action={<button className="rounded-xl bg-slate-950 px-4 py-2 text-sm font-bold text-white">New Workflow</button>} />
