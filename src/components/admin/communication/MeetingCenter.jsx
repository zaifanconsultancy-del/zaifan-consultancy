// MeetingCenter V4 PARTNER-OS STANDARD
// Full replacement for: src/components/admin/communication/MeetingCenter.jsx
...
function isOverdue(record = {}) {
  const date = new Date(getMeetingDate(record));
  return (
...
export default function MeetingCenter({
  ...
  const records = useMemo(() => safeArray(appointments), [appointments]);
  const meetingData = communicationData?.meetings || {};
  const telemetryConnected = Boolean(
    meetingData.metrics && Object.keys(meetingData.metrics).length
  );

  const stats = useMemo(
    () => ({
      total: records.length,
      today: records.filter((record) => isToday(getMeetingDate(record))).length,
  ...
  const visibleRecords = useMemo(() => {
    ...
    return records.filter((record) => {
      const matches =
        filter === "All" ||
        (filter === "Today" && isToday(getMeetingDate(record))) ||
        (filter === "Open" && !isClosed(record)) ||
        (filter === "Completed" && isClosed(record)) ||
  ...
  if (compact) {
    return (
      <MetricCard
        label="Appointments"
        value={stats.total}
        helper="Real appointment records."
        tone="navy"
        icon={Video}
      />
    );
  ...
  return (
    <Shell
      icon={Video}
      osLabel="Meeting OS"
      title="Meeting & Counseling Command"
      description="Manage genuine counseling appointments through one focused operating workspace. No placeholder meetings, attendance rates or completion metrics are generated."
      workspace="Meeting Operations"
      adminProfile={adminProfile}
      recordsCount={stats.total}
      secondaryCount={
        telemetryConnected ? "Telemetry connected" : "Appointments-table mode"
      }
    >
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="Appointments" value={stats.total} helper="Real appointment records supplied." tone="navy" icon={CalendarDays} badge="Schedule" />
        <MetricCard label="Today" value={stats.today} helper="Appointments scheduled for today." tone="blue" icon={Clock3} badge="Today" />
        <MetricCard label="Open" value={stats.open} helper="Appointments not completed or cancelled." tone="orange" icon={Users} badge="Queue" />
        <MetricCard label="Overdue" value={stats.overdue} helper="Open appointments whose scheduled date has passed." tone={stats.overdue ? "red" : "green"} icon={AlertTriangle} badge="Pressure" />
      </div>

      <section className="rounded-[1.5rem] border-[3px] border-[#C9D7E6] bg-white p-4 sm:p-5">
        <div className="mb-4 flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
          <div>
            <p className="text-[9px] font-black uppercase tracking-[0.11em] text-orange-700">Meeting Command</p>
            <h2 className="mt-1 text-xl font-black text-[#10233F]">Appointment portfolio</h2>
            <p className="mt-1 text-xs font-semibold leading-5 text-slate-600">One dominant workspace for counseling and consultation records.</p>
          </div>
          ...
          <div className="grid gap-2 sm:grid-cols-[auto_minmax(14rem,1fr)_auto]">
            ...
            <label className="relative block">
              ...
              <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search appointments..." className="min-h-10 w-full rounded-xl border-2 border-[#C9D7E6] bg-[#FFF8EF] pl-9 pr-3 text-xs font-semibold text-[#10233F]" />
            </label>
          ...
          {visibleRecords.length ? (
            visibleRecords.map((record, index) => (
              <article key={record.id || `${getName(record)}-${index}`} className="rounded-[1.3rem] border-2 border-[#C9D7E6] bg-white p-4 shadow-[0_6px_18px_rgba(15,35,63,0.04)] transition hover:border-[#F97316]">
                <div className="grid gap-4 xl:grid-cols-[minmax(18rem,1.35fr)_minmax(9rem,0.7fr)_minmax(9rem,0.7fr)_minmax(10rem,0.8fr)_minmax(10rem,0.8fr)] xl:items-center">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                    ...
                    <p className="mt-1 truncate text-[11px] font-semibold text-slate-500">{record.email || record.phone || "No contact supplied"}</p>
                  </div>

                  <div className="rounded-xl border border-[#E1E8F0] bg-[#FFF8EF] px-3 py-2.5"><p className="text-[7px] font-black uppercase text-slate-500">Date</p><p className="mt-1 text-xs font-black text-[#10233F]">{formatDate(getMeetingDate(record))}</p></div>
                  <div className="rounded-xl border border-[#E1E8F0] bg-[#FFF8EF] px-3 py-2.5"><p className="text-[7px] font-black uppercase text-slate-500">Time</p><p className="mt-1 text-xs font-black text-[#10233F]">{record.appointment_time || record.time || "Not recorded"}</p></div>
                  <div className="rounded-xl border border-[#E1E8F0] bg-[#FFF8EF] px-3 py-2.5"><p className="text-[7px] font-black uppercase text-slate-500">Type</p><p className="mt-1 truncate text-xs font-black text-[#10233F]">{record.consultation_type || "Consultation"}</p></div>
                  ...
                  <div className="col-span-full flex flex-wrap items-center justify-end gap-2 border-t-2 border-[#E7EDF4] pt-3">
                    ...
                    <button type="button" onClick={() => onOpenMeeting(record)} disabled={typeof onOpenMeeting !== "function"} className="inline-flex min-h-9 items-center gap-2 rounded-xl border-2 border-[#123865] bg-[#123865] px-3 text-[10px] font-black text-white disabled:opacity-40"><Video size={13} /> Open Meeting</button>
                  </div>
                </div>
              </article>
            ))
            ...
            <div className="rounded-[1.4rem] border-[3px] border-dashed border-[#C9D7E6] bg-[#FFF8EF] p-8 text-center">
              <Video size={24} className="mx-auto text-orange-700" />
              <p className="mt-3 font-black text-[#10233F]">{records.length ? "No appointments match these filters." : "No real appointments yet."}</p>
            </div>
          )}
        </div>
      </section>
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        <IntegrityCard icon={ShieldCheck} eyebrow="Schedule Integrity" title="No placeholder meetings" helper="Only real appointment records appear." tone="green" />
        <IntegrityCard icon={Database} eyebrow="Telemetry Boundary" title={telemetryConnected ? "Meeting telemetry connected" : "Appointments-table mode"} helper="Attendance and outcome analytics remain absent until real event data exists." tone="blue" />
        <IntegrityCard icon={CheckCircle2} eyebrow="Action Boundary" title={typeof onOpenMeeting === "function" ? "Meeting action connected" : "Meeting handler not connected"} helper="Actions remain disabled until a genuine handler is supplied." tone={typeof onOpenMeeting === "function" ? "green" : "amber"} />
      </div>
    </Shell>
  );
}
