...
const liveCountry = {
  ...
  ecosystem: [
    ...
    },
    {
      ...
      href: "/appointment?country=Italy&service=Italy%20Planning",
      cta: "Plan My Route",
    },
  ],
};
...
const italyDepthCards = [
  { icon: Compass, title: "Country Guide", text: "A complete Italy destination hub instead of a shallow country card." },
  { icon: Building2, title: "City System", text: "Milan, Rome, Bologna, Padua, Florence, Turin, Pisa and Venice connected." },
  { icon: GraduationCap, title: "University Database", text: "Smart university finder plus individual detail pages for deeper decisions." },
  { icon: WalletCards, title: "Funding Route", text: "DSU, regional scholarships, merit awards and document planning direction." },
  { icon: FileCheck2, title: "Document Strategy", text: "Application, scholarship, pre-enrolment and visa readiness connected." },
  { icon: Plane, title: "Guided Action", text: "Move from research to consultation, shortlisting and application planning." },
];

const roadmapCountries = [
  { name: "Germany", flag: "🇩🇪", code: "DE", image: germanyImage, priority: "Next Architecture", focus: "Public universities, engineering, low-tuition routes and practical career planning.", modules: ["Country Guide", "Cities", "Universities", "Funding", "Visa"] },
  { name: "United Kingdom", flag: "🇬🇧", code: "GB", image: ukImage, priority: "Coming Soon", focus: "Fast-track degrees, university scholarships, strong postgraduate routes and city-based planning.", modules: ["University Finder", "Scholarships", "Intakes", "Cost Guide"] },
  { name: "Canada", flag: "🇨🇦", code: "CA", image: canadaImage, priority: "Coming Soon", focus: "Colleges, universities, province-based routes and post-study planning when data is ready.", modules: ["Colleges", "Universities", "Province Guide", "Funding"] },
  { name: "Australia", flag: "🇦🇺", code: "AU", image: australiaImage, priority: "Coming Soon", focus: "University grants, regional study options, lifestyle planning and future student pathways.", modules: ["Cities", "Universities", "Scholarships", "Lifestyle"] },
  { name: "Turkey", flag: "🇹🇷", code: "TR", image: turkeyImage, priority: "Future Route", focus: "Affordable education, institutional awards and future partner university routes.", modules: ["Affordable Routes", "Partners", "Scholarships", "Guide"] },
];

const decisionPoints = [
  { icon: Compass, title: "Choose with confidence", text: "Start with a destination that has real guidance instead of a fake country database." },
  { icon: Route, title: "Follow a connected journey", text: "Country → city → university → scholarship → consultation should feel like one ecosystem." },
...
const italyJourney = [
  ...
  { icon: GraduationCap, title: "Shortlist Universities", text: "Filter universities by program, scholarship strength, tuition and student fit.", href: "/universities" },
  { icon: WalletCards, title: "Plan Scholarships", text: "Check DSU, regional grants, merit awards and document preparation.", href: "/scholarships" },
  { icon: Plane, title: "Book Guidance", text: "Turn research into an application, scholarship and visa preparation plan.", href: "/appointment?country=Italy" },
];
...
const hoverCard =
  "transition duration-300 ease-out hover:-translate-y-1 hover:shadow-[0_24px_60px_rgba(255,75,18,0.12)]";
  ...
  return (
    <section className="relative overflow-hidden bg-[linear-gradient(180deg,#fff8f1_0%,#fff5e9_58%,#fff1e3_100%)] px-4 pb-20 pt-28 text-[#071f50] sm:px-5 sm:pb-24">
      ...
      <div className="relative mx-auto max-w-[1600px]">
        <motion.div initial="hidden" whileInView="show" variants={fadeUp} viewport={{ once: true, amount: 0.2 }} className="mb-10 text-center">
            ...
            <div className="grid md:grid-cols-[1.05fr_1.15fr]">
              <div className="relative min-h-[360px] overflow-hidden bg-orange-50 sm:min-h-[410px] md:h-full">
                ...
                <div className="absolute left-6 top-6 inline-flex items-center gap-2 rounded-full bg-white/95 px-4 py-2 text-xs font-black uppercase tracking-[0.16em] text-[#ff4b12] shadow-lg">
                  <BadgeCheck size={15} /> Live Destination
                </div>
                <div className="absolute bottom-6 left-6 right-6 rounded-[28px] bg-white/94 p-5 shadow-xl backdrop-blur">
                  <p className="text-xs font-black uppercase tracking-[0.2em] text-[#ff4b12]">Featured Country</p>
                  <h3 className="mt-2 text-3xl font-black tracking-[-0.055em] text-[#071f50] md:text-4xl">{liveCountry.flag} {liveCountry.title}</h3>
                </div>
              </div>

              <div className="flex min-w-0 flex-col p-7 md:p-9">
                <div className="inline-flex w-fit items-center gap-2 rounded-full bg-[#fff1ea] px-4 py-2 text-xs font-black uppercase tracking-[0.18em] text-[#ff4b12] ring-1 ring-orange-100"><MapPin size={15} /> Best place to start</div>
                <h2 className="mt-5 text-4xl font-black leading-[0.96] tracking-[-0.055em] md:text-5xl">Your complete Italy journey starts here.</h2>
                <p className="mt-5 text-[15px] font-semibold leading-8 text-[#526178]">{liveCountry.text}</p>

                <div className="mt-6 grid gap-3 sm:grid-cols-2">
                  {liveCountry.stats.map(([label, value]) => (
                    <div key={label} className={`rounded-2xl bg-[#fffaf5] px-4 py-4 ring-1 ring-orange-100 ${hoverCard}`}>
                      <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#ff4b12]">{label}</p>
                      <p className="mt-1 text-xl font-black text-[#071f50]">{value}</p>
                    </div>
                  ))}
                </div>

                <div className="mt-7 grid gap-3 sm:grid-cols-2">
                  <Link to={liveCountry.href} className="inline-flex items-center justify-center gap-3 rounded-full bg-[#ff4b12] px-6 py-4 text-sm font-black text-white shadow-[0_18px_38px_rgba(255,75,18,0.28)] transition duration-300 ease-out hover:-translate-y-1 hover:bg-[#ff642f] focus:outline-none focus:ring-4 focus:ring-orange-200">
                    Explore Italy Guide <ArrowRight size={19} strokeWidth={3} />
                  </Link>
                  <Link to="/appointment?country=Italy" className="inline-flex items-center justify-center gap-3 rounded-full bg-white px-6 py-4 text-sm font-black text-[#ff4b12] ring-1 ring-orange-200 transition duration-300 ease-out hover:-translate-y-1 hover:bg-[#fff1ea] focus:outline-none focus:ring-4 focus:ring-orange-100">
              ...
              <div className="border-t border-orange-100 bg-[#fffaf5] p-6 md:col-span-2 md:p-7">
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {italyDepthCards.map((item) => {
                    ...
                    return (
                      <div key={item.title} className={`rounded-[22px] bg-white p-4 shadow-sm ring-1 ring-orange-100 ${hoverCard}`}>
                        <div className="flex items-start gap-3">
                          <div className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-[#fff1ea] text-[#ff4b12] ring-1 ring-orange-100"><Icon size={21} strokeWidth={2.7} /></div>
                          <div><h4 className="text-sm font-black text-[#071f50]">{item.title}</h4><p className="mt-1 text-xs font-semibold leading-5 text-[#61708a]">{item.text}</p></div>
                        </div>
                      </div>
                    );
                  })}
          ...
          <motion.aside initial={{ opacity: 0, x: 24 }} whileInView={{ opacity: 1, x: 0 }} transition={MOTION} viewport={{ once: true, amount: 0.15 }} className="rounded-[42px] bg-[#071f50] p-7 text-white shadow-[0_32px_90px_rgba(7,31,80,0.18)]">
            ...
            <div className="mt-7 space-y-4">
              {decisionPoints.map((item) => {
                const Icon = item.icon;
                return (
                  <div key={item.title} className="rounded-[26px] bg-white/8 p-5 ring-1 ring-white/10 transition duration-300 ease-out hover:-translate-y-1 hover:bg-white/12">
                    <div className="flex gap-4"><div className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-[#ff4b12] text-white"><Icon size={22} strokeWidth={3} /></div><div><h4 className="font-black text-white">{item.title}</h4><p className="mt-2 text-sm font-semibold leading-6 text-white/68">{item.text}</p></div></div>
                  </div>
                );
              })}
            </div>
            <div className="mt-6 rounded-[26px] bg-white/10 p-5 ring-1 ring-white/10">
              <p className="text-xs font-black uppercase tracking-[0.18em] text-[#ffb36d]">Current recommendation</p>
        ...
        <section className="mx-auto mt-10 max-w-[1450px] rounded-[42px] bg-white/88 p-6 shadow-[0_24px_70px_rgba(9,31,80,0.08)] ring-1 ring-orange-100 md:p-8">
          ...
          <div className="relative grid gap-5 md:grid-cols-2 xl:grid-cols-4 xl:before:absolute xl:before:left-[10%] xl:before:right-[10%] xl:before:top-7 xl:before:h-px xl:before:bg-orange-200">
            {liveCountry.ecosystem.map((item) => {
              const Icon = item.icon;
              return (
                <Link key={item.title} to={item.href} className="group rounded-[30px] bg-[#fffaf5] p-6 ring-1 ring-orange-100 transition duration-300 ease-out hover:-translate-y-1 hover:bg-white hover:shadow-[0_24px_60px_rgba(255,75,18,0.12)]">
                  <div className="mb-5 flex items-start justify-between gap-3"><div className="grid h-14 w-14 place-items-center rounded-2xl bg-white text-[#ff4b12] ring-1 ring-orange-100"><Icon size={27} strokeWidth={2.7} /></div><ArrowRight size={20} strokeWidth={3} className="text-[#ff4b12] transition duration-300 group-hover:translate-x-1" /></div>
                  <h4 className="text-xl font-black tracking-[-0.035em] text-[#071f50]">{item.title}</h4><p className="mt-3 text-sm font-semibold leading-7 text-[#526178]">{item.text}</p><p className="mt-5 text-xs font-black uppercase tracking-[0.15em] text-[#ff4b12]">{item.cta}</p>
                </Link>
              );
            })}
          </div>
        ...
        <section className="mx-auto mt-10 max-w-[1450px] rounded-[42px] bg-[#071f50] p-6 text-white shadow-[0_30px_90px_rgba(7,31,80,0.18)] md:p-8">
          <div className="mb-8 text-center"><div className="inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 text-xs font-black uppercase tracking-[0.18em] text-[#ffb36d] ring-1 ring-white/10"><Plane size={15} /> Student journey</div><h3 className="mx-auto mt-4 max-w-4xl text-4xl font-black leading-tight tracking-[-0.05em] md:text-5xl">From country research to real action.</h3></div>
        ...
        <section className="mx-auto mt-10 max-w-[1450px] rounded-[42px] bg-white/88 p-6 shadow-[0_24px_70px_rgba(9,31,80,0.08)] ring-1 ring-orange-100 md:p-8">
          <div className="mb-7 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div><div className="inline-flex items-center gap-2 rounded-full bg-[#fff1ea] px-4 py-2 text-xs font-black uppercase tracking-[0.18em] text-[#ff4b12] ring-1 ring-orange-100"><Star size={15} /> Country readiness comparison</div><h3 className="mt-4 text-4xl font-black leading-tight tracking-[-0.05em]">What is ready now vs what is still coming.</h3></div>
            <Link to="/countries/italy" className="inline-flex items-center justify-center gap-3 rounded-[20px] bg-[#ff4b12] px-7 py-4 text-sm font-black text-white shadow-[0_18px_38px_rgba(255,75,18,0.26)] transition duration-300 ease-out hover:-translate-y-1 hover:bg-[#ff642f]">Start With Italy <ArrowRight size={18} strokeWidth={3} /></Link>
          </div>
        ...
        <section className="mx-auto mt-10 max-w-[1450px] rounded-[42px] bg-white/84 p-6 shadow-[0_24px_70px_rgba(9,31,80,0.08)] ring-1 ring-orange-100 backdrop-blur md:p-8">
          <div className="mb-7 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div><div className="inline-flex items-center gap-2 rounded-full bg-[#fff1ea] px-4 py-2 text-xs font-black uppercase tracking-[0.18em] text-[#ff4b12] ring-1 ring-orange-100"><Hourglass size={15} /> Country expansion roadmap</div><h3 className="mt-4 text-4xl font-black leading-tight tracking-[-0.05em]">More countries are visible — but honestly marked coming soon.</h3></div>
            <Link to="/appointment?service=Request%20Country%20Guide" className="inline-flex items-center justify-center gap-3 rounded-[20px] bg-[#071f50] px-7 py-4 text-sm font-black text-white shadow-[0_18px_38px_rgba(7,31,80,0.18)] transition duration-300 ease-out hover:-translate-y-1 hover:bg-[#092b72]">Request Country Guide <ArrowRight size={18} strokeWidth={3} /></Link>
          </div>
